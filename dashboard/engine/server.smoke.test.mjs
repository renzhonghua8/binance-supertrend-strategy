...
test('engine exposes safe default state and validates config',async()=>{
 ...
 try{await waitForServer();const snapshot=await fetch(`http://127.0.0.1:${port}/api/snapshot`).then(r=>r.json());assert.equal(snapshot.connected,false);assert.equal(snapshot.running,false);assert.equal(snapshot.mode,'paper');assert.equal(snapshot.config.maxRiskPct,20);assert.equal(snapshot.config.maxStopDropPct,10);assert.equal('leverage'in snapshot.config,false);assert.equal('exposure'in snapshot.config,false);

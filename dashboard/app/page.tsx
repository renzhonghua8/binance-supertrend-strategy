...
export default function Home(){
 ...
 return <main>
  ...
  <div className="shell">
   ...
   <section className="watch card"><div className="card-title"><div><p>BREAKOUT WATCH</p><h2>重点观察 · 即将突破</h2></div><span>{watchlist.length} 个候选</span></div>{watchlist.length?<div className="watch-grid">{watchlist.map((r,i)=><article key={r.symbol} className={i===0?'nearest':''}><div className="watch-head"><span>榜单 #{r.rank}</span><b>{i===0?'距离最近':'等待突破'}</b></div><h3>{r.symbol.replace('USDT','')}<small>/USDT</small></h3><div className="watch-prices"><div><span>5m收盘</span><strong>{num(r.signals['5m']?.close??null,6)}</strong></div><div><span>突破参考线</span><strong>{num(r.trigger,6)}</strong></div></div><div className="distance"><span><i style={{width:`${Math.max(8,100-Math.min(100,r.gapAtr*50))}%`}}/></span><b>还差 {r.gapAtr.toFixed(2)} ATR</b></div><p>价差 {num(r.gapPrice,6)} · {r.gapPct.toFixed(2)}%</p></article>)}</div>:<div className="empty">当前没有“高周期全部合格且5分钟仍在线下”的币种。列表会随每次榜单更新自动变化。</div>}<footer className="watch-note">观察条件：15m、1h、4h、1d全部在线上，5m仍在线下；按距离5m Supertrend线的ATR倍数排序。实际入场仍需等待已完成5m收盘正式站上线。</footer></section>
